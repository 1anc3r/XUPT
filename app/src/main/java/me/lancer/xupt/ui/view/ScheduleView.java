package me.lancer.xupt.ui.view;
...
import me.lancer.xupt.util.DensityUtil;

import static me.lancer.xupt.R.*;
import static me.lancer.xupt.R.style.ScheduleView;
...
public class ScheduleView extends RelativeLayout {
    ...
    private List<? extends CourseBean> arrSchedule;
    private String[] CN_DAYS = {"周一", "周二", "周三", "周四", "周五", "周六", "周日"};
    private int[] US_DAYS = {5, 6, 0, 1, 2, 3, 4};
    ...
    public ScheduleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, styleable.Schedule, defStyleAttr,
                0);
    ...
    private void drawMonthView() {
        ...
        llMonth = new LinearLayout(getContext(), null, style.ScheduleView);
    ...
    private void drawWeekView() {
        ...
        for (int i = 0; i < totalDay; i++) {
            ll = new LinearLayout(getContext(), null, style.ScheduleView);
    ...
    private void drawScheduleFrame() {
        for (int i = 0; i < totalDay * totalTime; i++) {
            ...
            FrameLayout fl = new FrameLayout(getContext(), null, style.ScheduleView);
    ...
    public void updateSchedule(List<? extends CourseBean> arrSchedule) {
    ...
    private void updateSchedule() {
        ...
        TextView tvSchedule;
        for (final CourseBean c : arrSchedule) {
            final int time = c.getCourseTime();
            final int day = c.getCourseDay();
            fl = new FrameLayout(getContext());
            flp = new FrameLayout.LayoutParams(squareWidth, squareHeight * 2);
            ...
            tvSchedule.setBackgroundResource(Schedule_BG[c.getCourseColor()]);
            tvSchedule.setLayoutParams(flp);
            tvSchedule.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (onScheduleItemClickListener != null)
                        onScheduleItemClickListener.onScheduleItemClick((TextView) v, time, day, c.getCourseName());
                }
            });
